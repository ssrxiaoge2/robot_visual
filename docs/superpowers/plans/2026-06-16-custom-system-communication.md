...
> **历史方案状态（2026-07-13）：已废弃，禁止作为现行实施依据。** 旧“客户系统通信测试” UI、`.229` 地址、`DayRecord`、测试连接/读取数据诊断 API 和 DeviceManager 旧入口已由 `2026-07-13-live-shortage-ledger.md` 的真实缺料方案删除。`customSysScheduler.h/.cpp` 仅保留文件名并原地改造成真实缺料唯一 MES/PLC 协议层；不得按本文重新接回旧面板、旧地址或兼容空壳。
