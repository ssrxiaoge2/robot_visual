...
7. 按 `目标TCP Z = (目标层表面离地高度 + releaseZOffset) - robotBaseHeightFromGround + PALLET_GRIPPER_RELEASE_Z_OFFSET_MM`、`deltaZ = 目标TCP Z - Z_basePoint` 计算 Z 相对移动量；若 `目标TCP Z > Z_basePoint`，说明本次不是下放而是向上抬升，必须拒绝执行，避免高层接近机械臂奇异点。
...
- `Z 相对移动量` 不允许为正值；目标 TCP Z 高于码垛初始点位 TCP Z 时，`HuayanScheduler` 在下发 Z 轴动作前直接报错，不提交 `placedCount`。
...
- 方向反转、已放数量展示、人工修正、人工清零。
- 容量、下一点、点位表、校验结果。
- “执行一次码垛”按钮。

配置页不再提供“最高安全 Z”和“机械臂初始点位绝对预览”。码垛初始点位由示教器函数 `palletBaseFunc` 管理，上位机在函数到位后读取真实 TCP Z；这个实时 TCP Z 就是本次动作的高位保护参考。配置页的点位表只显示相对偏移、目标层表面离地高度、释放地面 Z 和目标 TCP Z 估算值。
...
- 运行时若目标 TCP Z 高于码垛初始点位 TCP Z，`HuayanScheduler` 返回失败，提示降低最大层数、释放高度或修正现场高度参数。
- 执行期间按钮禁用，避免并发启动。
...
2026-07-10 单独码垛测试基本满足现场要求后，可以进入主流程联调。当前阶段满垛、空垛尚未和外部系统通信，因此主流程测试暂时使用“人工确认 + 配置页手动清零/修正数量”替代外部信号。

允许的临时测试方式：

- 主流程开始前，操作员先确认托盘现场状态，并在空箱码垛配置页把 `placedCount` 修正为现场真实已放数量。
- 如果托盘区域为空，必须手动清零，使下一次主流程从第一点开始。
- 如果满垛后人工已经搬走空箱，并且空托盘已经就位，可以在配置页手动清零，模拟后续外部系统的“满垛已搬走/空垛已就位”信号。
- 如果托盘上仍有箱子，不允许为了继续测试而随意清零；否则主流程会按第一层点位重新放置，存在撞箱或落点错误风险。

现场 Z 误差说明：

- 机器人基座离地高度可能存在测量误差，车体高度、地面不平也会影响实际释放高度。
- 夹爪长度和 TCP/法兰到释放参考点的补偿值存在测量误差。
- 框体高度存在测量误差，且不同框体可能因变形、磨损导致实际高度不一致。
- 因上述误差，`releaseZOffset` 并不一定表现为“刚好比箱体高度多 10mm 后释放”。现场观察到第一层释放点可能接近第一层中部，高层误差可能累积变大。只要单独码垛测试落箱效果仍在可接受范围内，主流程可按同一套动作链继续联调。

主流程联调建议：

- 第一次只跑 1 到 3 次码垛，确认倒料后夹爪确实保持夹紧、机械臂处于安全位、主流程调用 `startPalletPlaceFromClampedSafety()` 跳过重复夹紧。
- 每次成功后确认 `CommitPallet` 才推进 `placedCount`，失败、停止、满载拒绝时不得增加数量。
- 观察主流程日志中的 `基准TCP Z`、`目标TCP Z`、`释放地面Z`、`本次Z相对移动`，确认与单独码垛测试表现一致。
- 在外部满垛/空垛通信接入前，所有清零都必须由人工确认现场已经搬空并复位托盘。
...
- 满载时不能继续计算或执行。
- 人工清零后重新从第一点开始。
- 配置页机械臂码垛动作规划顺序为：夹紧、基准点、XY、Z下降、松爪、Z抬升、回安全位。
- 主流程进入码垛时空箱已夹紧，因此调用 `HuayanScheduler::startPalletPlaceFromClampedSafety()` 跳过重复夹紧，其余动作链完全复用。

建议新增纯规划 helper 测试动作顺序，避免单元测试直接依赖真实华研 SDK。

## 现场待分析问题

2026-07-09 现场实测发现：空垛调试时，下面两层肉眼观察下降高度基本合理，但执行到第 5 层附近时，机械臂读取/运动到的 Z 值异常偏高，接近或达到机械臂奇异点并导致动作挂起。2026-07-10 已按现场结论修正：UI 不再让人工填写“最高安全 Z”，运行时改用 `palletBaseFunc` 到位后读取的当前 TCP Z 作为高位保护参考；目标 TCP Z 高于该基准点时拒绝执行。

待重点分析：

- `placedCount`、层号、行列计算是否在连续单步执行中与现场层数一致。
- `palletSize.z` 当前作为“托盘面离地高度”是否和现场配置值一致。
- `releaseZOffset` 是否仍偏大，导致高层释放点地面高度过高；高层调试必须观察是否触发“目标 TCP Z 高于码垛初始点位”保护。
...
- 是否需要继续扩展日志，把每次码垛的 `层/行/列、目标层表面离地高度、释放地面高度、当前 TCP Z、目标 TCP Z、计算 deltaZ、动作后 TCP Z` 写入更完整的调试日志。

明日讨论前，现场应保留第 1 层到第 5 层每次执行时的码垛日志，尤其是：

```text
[码垛] 基准TCP Z=...，目标TCP Z=...，释放地面Z=...，机器人基座离地=...，夹爪释放补偿=...，本次Z相对移动=...
```
...
12. 配置页单步稳定后，再启动主流程，确认 `PreparePalletPoint -> ArmPalletPlace -> CommitPallet` 使用同一套配置和动作，并确认主流程日志显示已夹紧时跳过重复夹紧。
