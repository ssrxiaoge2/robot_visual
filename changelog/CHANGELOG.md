...
### 新增
- 主界面“复位”右侧新增“设置”入口，按六类集中展示可调整的运行参数。
- 设置界面支持分类恢复默认值、旧值到新值预览及安全确认；设备运行期间自动切换为只读，避免误改参数。
- 新增视觉深度自动下探：目标深度大于 `1200 mm` 时，相机每次下探 `200 mm` 后重新检测，累计下探上限为 `400 mm`，且与无目标搜索流程相互独立。

### 变更
- 大筐抓取 Z 余量支持运行时配置，1～11 号工位默认值为 `417 mm`；12 号紫筐抓取 Z 余量默认值为 `380 mm`。
- 调度器、视觉检测及生产流程统一读取运行时设置，参数修改无需重新编译。

### 安全与持久化
- 参数仅允许在设备空闲时应用，运行中保持只读。
- 设置使用 INI 文件持久化并采用原子写入；应用失败时回滚到修改前的有效配置。

### 验证
- 通过 MSVC Release 全量配置与构建。
- CTest 自动化测试 `16/16` 通过。
- 实机机械臂与现场视觉流程仍需按部署环境完成验收。
...
### 新增
- 引入完整 N-ScanHub Linux/Windows SDK，CMake 根据平台和架构配置头文件、动态库、运行时依赖及安装规则
- 新增 `NScanScheduler` 同步封装，提供参数校验、连接复用、软件触发、限时读取、失败重试、条码解码和结构化结果
- 新增厂家 SDK 独立网络验证程序、Makefile 和运行脚本，默认使用现场验证成功的十六进制触发指令 `41`
- 主窗口新增独立的“N-ScanHub SDK 调试”面板，支持配置 IP、端口、单次超时、最大尝试和重试间隔，并显示条码、尝试次数、原始数据和成功计数

### 变更
- `DeviceManager` 统一接入扫码调度，通过单一复用工作线程执行同步 SDK 调用；线程空闲时等待事件，高频扫码不再反复创建和销毁线程
- 扫码参数控件使用 `QLineEdit + QIntValidator`，触发前再次检查空值和范围，非法参数直接在界面及日志提示
- 扫码面板状态色接入主题切换，扫码期间禁用参数和触发按钮，任务完成后恢复
- 成功连接跨扫码任务复用；连接、发送或读取失败时关闭连接并按配置重新建立

### 验证
- 厂家独立验证程序在扫码枪 `192.168.1.12:30000` 上完成连接、发送 `41` 和读取条码验证
- 上位机主动扫码真机测试成功，扫码数据读取正确
- Linux Release 与 Qt Debug 构建通过，验证工具通过 `-Wall -Wextra -Wpedantic` 编译
- 复用工作线程空闲启动和退出测试通过，未出现线程销毁告警

### 已知限制
- 厂家 SDK 的同步连接/读取调用无法安全强制中断；若 SDK 内部永久阻塞，程序退出只能执行有界等待并记录警告
- MinGW 缺少厂家导入库，当前仅启用 Linux 和 Windows MSVC SDK 路径
...
- 取料后收姿态不动：阶段完成信号 `stageCompleted` 直连编排器，在 `emit` 内同步启动下一阶段（收姿态），而原 `None` 分支在 `emit` 之后才调 `stop()`，把刚启动的收姿态阶段 `m_stage` 重置为 `None`，延时 `proceedStage` 因此被跳过。抽出 `completeStage()` 改为先 `stop()` 清理本阶段再 `emit`，五处阶段收尾统一改用
