...
4. 同层多个箱子高度接近时，按固定侧规则选择；当前默认按 raw `Y` 最小选择，现场可通过 `VISION_LOCK_FIXED_SIDE_PICK_MIN` 调整方向。
5. 目标一旦锁定，闭环微调过程中只允许继续跟踪锁定目标附近候选；如果本帧只识别到旁边工位目标，本帧不下发 `MoveRelL`。
...
阶段一单次 X/Y 微调硬保护 `HUAYAN_MAX_SINGLE_XY_ADJUST_MM` 当前为 `250.0mm`。该值用于防止异常目标导致过大 `MoveRelL`，不是用于解决控制器安全空间错误。
