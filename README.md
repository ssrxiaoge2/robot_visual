...
- 视觉计算出的 Rz 若出现 80° 以上大角度跳变，必须连续两帧同向确认后才允许执行旋转；同一目标锁定周期内大角度 Rz 最多实际执行一次，后续重复 90° 视为疑似视觉旧帧或角度歧义并跳过 Rz，避免累计旋转。
...
阶段一取料不再按“当前帧全局最高目标”每帧重新选择，而是采用现场验证通过的“初始锁定 + 闭环跟踪”策略：

1. 机械臂先到当前工位的 `Func_captureN` 拍照位，该拍照位作为本轮固定锚点。
2. 初始帧先过滤超出拍照锚点 X/Y 矩形可信范围的候选，旁边工位目标即使视觉判断更高，也不能参与当前工位目标竞争。
3. 在可信候选中按 Z 最高优先选择，避免低层箱子先被抓取时碰到夹爪。
4. 同层多个箱子高度接近时，初始帧按固定侧规则选择；当前默认按 raw `Y` 最小选择，现场可通过 `VISION_LOCK_FIXED_SIDE_PICK_MIN` 调整方向。
5. 目标一旦锁定，闭环微调过程中不再按固定侧 Y 重新选择；先在锁定目标附近候选中按 Z 最高层过滤，再在同层内选择 `lockdist` 最小者，避免为了连续性抓到较低层箱子。
6. 锁定目标连续丢失达到 `VISION_LOCK_MAX_MISSING_FRAMES` 后，阶段一失败并停止，禁止自动切换到旁边工位目标。

该方法依赖两个现场条件：工位料箱摆放应尽量稳定，`Func_captureN` 拍照点位应设置在当前工位正中间附近。若料箱摆放偏差过大或拍照位明显偏离工位中心，目标锁定范围会被带偏，选择效果会下降。

关键宏位于 `src/visionclient.h`：

| 宏 | 当前值 | 说明 |
|----|--------|------|
| `VISION_ANCHOR_MAX_TRUST_X_MM` | `300.0` | 初始拍照锚点 X 方向最大可信偏移；用于过滤斜向进入旧圆形范围的旁站目标 |
| `VISION_ANCHOR_MAX_TRUST_Y_MM` | `450.0` | 初始拍照锚点 Y 方向最大可信偏移；现场若轴向相反或正常目标被挡，可优先调整该值 |
| `VISION_LOCK_MAX_MISSING_FRAMES` | `3` | 锁定目标连续丢失 3 帧后失败 |
| `VISION_LOCK_TRACK_RADIUS_MM` | `260.0` | 闭环候选离锁定目标最大允许距离 |
| `VISION_LOCK_SAME_LAYER_Z_TOL_MM` | `80.0` | 锁定模式同层 Z 容差 |
| `VISION_LOCK_FIXED_SIDE_AXIS_Y` | `1` | 固定侧选择按 Y 轴 |
| `VISION_LOCK_FIXED_SIDE_PICK_MIN` | `1` | 固定选择 Y 最小；如现场方向反了可改为 `0` |

阶段一机械臂侧关键保护位于 `src/huayanScheduler.cpp`：

| 常量 | 当前值 | 说明 |
|------|--------|------|
| `HUAYAN_MAX_SINGLE_XY_ADJUST_MM` | `250.0` | 阶段一单次 X/Y 微调硬上限，防止异常目标导致过大 `MoveRelL` |
| `HUAYAN_STAGE_ONE_MAX_LARGE_RZ_EXECUTIONS` | `1` | 同一目标锁定周期内允许实际执行的大角度 Rz 次数 |
| `HUAYAN_STAGE_ONE_Z_DESCEND_TIMEOUT_MS` | `120000` | 阶段一 Z 下探专用到位等待超时，普通 X/Y/Rz 微调仍使用默认超时 |

这些值用于减少异常视觉结果和旧帧带来的错误动作，不宣称根治控制器内部安全空间错误。

### 华研 40961 / 49601 待厂家确认问题

现场仍可能出现华研控制器返回 `40961`（此前诊断日志中同类现象也记录过 `49601 Target orientation exceeded cartesian safety space`）。当前上位机能做的是：

- 通过目标锁定策略减少选错旁边工位导致的大幅 X/Y 微调。
- 在单次 X/Y 微调超过 `HUAYAN_MAX_SINGLE_XY_ADJUST_MM` 时拒绝下发 `MoveRelL`。
- 对重复出现的 `Rz=90°` 大角度结果做累计次数保护，避免视觉旧帧导致机械臂连续旋转。
- 在 SDK 拒绝 `MoveRelL` 时输出命令号、命令前 TCP、命令前关节角、命令后状态和各轴错误码，便于向华研厂家确认。

该问题目前不在上位机侧宣称根治，需要向华研确认错误码含义、触发条件，以及当前 TCP/姿态/工具系相对运动是否进入控制器笛卡尔安全空间限制。
...
| 4D 视觉联合对准 | 现场复测中 | 首次联合 MoveJ 到预抓取位，之后最多 0～2 次工具系 XY/Rz 联合 MoveL 精修；达到上限仍不收敛则安全停止 |
...
| 同层两个目标闭环摇摆 | 初始帧仍按固定侧 Y 定边，锁定后改按 `lockdist` 最近连续跟踪 |
| Rz=90° 重复累计旋转 | 大角度 Rz 保留双帧确认，并限制同一目标周期最多实际执行一次 |
| 1 米级 Z 下探 30 秒超时 | 阶段一 Z 下探使用 120 秒专用到位等待超时 |
| 取料后收姿态不动 | 阶段收尾先 `stop()` 再发完成信号，避免重置刚启动的下一阶段 |
| X、Y、Rz 分轴启停导致动作卡顿 | 首次目标使用一条联合 MoveJ 到预抓取位，残差使用有限次数的工具系 XY/Rz 联合 MoveL 精修 |
| XY/Rz 已满足但 Z 被异常帧反复打断 | 使用最近 5 个真实新帧，去掉一高一低后按核心 3 帧极差判稳，并使用 5 帧中值下探 |
| 锁定目标暂时丢失 | 保持立即安全停止，不使用旧坐标续抓、不切换旁站目标、不自动重试 |
...
| 真实夹爪 TCP 与手眼坐标链 | 当前原有 `TCP` 位于法兰零点；需标定夹爪抓取中心，统一迁移示教器 `Func_*` 和 C++ TCP 名称，确认 Modbus `1036` 语义后重新手眼标定，并复测大角度 Rz 的 XY 耦合 |
