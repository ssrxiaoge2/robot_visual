 ...
 *
 * 本类只负责编排高层动作：向 AGV 请求导航、调用 HuayanScheduler 阶段、请求扫码、
 * 查询/提交码垛点位。它不直接调用 Modbus 或华研 SDK 原语，也不管理 FIFO 队列。
 * 对象与设备控制器同属 UI 主线程；扫码结果由 DeviceManager 从 worker 线程排队转回。
 */
...
private:
    ...
    /// 从静态配置表解析当前工位及码垛区；失败时内部已发 systemError。
    ...
    /// 校验码垛配置并获取下一偏移；这里只读取，不推进 placedCount。
