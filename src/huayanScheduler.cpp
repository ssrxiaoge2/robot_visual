    ...
    : QObject(parent)
{
    ...
    connect(m_stageTimer, &QTimer::timeout,
            this, &HuayanScheduler::onStageTimerTimeout);
...
void HuayanScheduler::onStageTimerTimeout()
{
    advanceStep();
    proceedStage();
}
...
void HuayanScheduler::executeCurrentStep()
{
    ...
    case Stage::StageTwo:
        ...
        break;
    case Stage::StageThree:
        if (m_stageStep == StageStep::ExecuteStackingFunction) {
            emit logMessage(QStringLiteral("[阶段三] 执行码垛脚本函数"));
            if (executeStackingFunction()) {
                emit stageCompleted(stageName(m_stage));
                stop();
            }
        }
